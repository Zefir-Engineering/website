...
var department = region.substring(0, 2);

var list_postal_codes = [
  "59491",
  "59262",
  "59800",
  "59112",
...
document.getElementById("Home Address 1").innerHTML =
  HomeAddress[0] + " " + HomeAddress[1];
document.getElementById("Home Address 2").innerHTML =
...
function ImportDataSlide2(New_Adresse) {
  var URL =
    "https://geocodage.ign.fr/look4/address/search?&q=" +
    New_Adresse +
    "&returnTrueGeometry=true";

  axios.get(URL).then((resp) => {
    var coordinates = JSON.stringify(
      resp.data.features[0].properties.houseNumberInfos.otherPositions[0]
        .geometry
    );
    var URL_2 =
      "https://geocodage.ign.fr/look4/parcel/reverse?searchGeom=" + coordinates;

    axios.get(URL_2).then((resp) => {
      var Landref = resp.data.features[0].properties.identifiant;
      var URL_3 =
        "https://geocodage.ign.fr/look4/parcel/search?&q=" +
        Landref +
        "&returnTrueGeometry=true";
  ...
  function getSurfaceArea(json) {
    var coordinates = JSON.stringify(
      json.features[0].properties.trueGeometry.coordinates[0]
    );
    ...
    if (document.getElementById("apartment").checked) {
      ...
      document.getElementById("landsizesqft").value = Math.round(
        turf.area(polygons)
      );
    }
    ...
    $(document).ready(function () {
      if (
        $("#landsizesqft").val() <= 1000000 &&
        $("#landsizesqft").val() >= 1
      ) {
        document.getElementById("divlandsizesqft").style.borderColor =
          "#f8f6f4";
...
document.getElementById("startbutton").onclick = function () {
  ImportDataSlide2(New_Adresse);
};

$(document).ready(function () {
  $("#apartmentsizesqft").keyup(function () {
    if (
      $("#apartmentsizesqft").val() <= 2000 &&
      $("#apartmentsizesqft").val() >= 1
    ) {
      document.getElementById("divapartmentsizesqft").style.borderColor =
        "#f8f6f4";
      ...
      document.getElementById("divapartmentsizesqft").style.borderColor =
        "#ed6a5e";
      document.getElementById("error1apartmentsizesqft").style.display = "none";
      document.getElementById("error2apartmentsizesqft").style.display =
        "block";
      bool4 = false;
    } else {
      document.getElementById("divapartmentsizesqft").style.borderColor =
        "#ed6a5e";
      document.getElementById("error2apartmentsizesqft").style.display = "none";
      document.getElementById("error1apartmentsizesqft").style.display =
        "block";
      bool4 = false;
    }
  });
...
$(document).ready(function () {
  $("#formcontentslide7").click(function () {
    var buttonslide7 = document.getElementById("buttonslide7");
    if (
      $("input[id='checkboxslide8']:checked").val() ||
      $("input[id='checkbox-norareissues']:checked").val() ||
      $("input[id='checkbox-otherlead']:checked").val()
    ) {
